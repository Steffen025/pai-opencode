...
```text
□ CRITICAL: This is PAI-OpenCode. Working directory is ~/.opencode/ NOT ~/.claude/
    → CORRECT: ~/.opencode/
    → WRONG:   ~/.claude/ or ~/.Claude/

□ IMPORTANT: ~/.claude/ context
    → OpenCode MAY READ from ~/.claude/ (skill/resource discovery for Claude Code compat)
    → OpenCode must NEVER WRITE to ~/.claude/
    → On some systems ~/.claude/ is Claude Code's directory (separate from ~/.opencode/)
    → On other systems ~/.claude → ~/.opencode (symlink) — but don't rely on this

□ Check every file operation path before executing
    → Memory:  ~/.opencode/MEMORY/
    → Skills:  ~/.opencode/skills/ (user) or .opencode/skills/ (project)
    → PRDs:    ~/.opencode/MEMORY/WORK/{session-slug}/
    → Config:  opencode.json (project root) + ~/.opencode/settings.json

□ If files were accidentally written to ~/.claude/ (and ~/.claude is NOT a symlink):
    → Detect:  ls ~/.claude/MEMORY/ — if files exist that belong in ~/.opencode/
    → Backup:  cp -r ~/.claude/MEMORY/ ~/.claude/MEMORY.bak/
    → Migrate: rsync -av ~/.claude/MEMORY/ ~/.opencode/MEMORY/
    → Verify:  ls ~/.opencode/MEMORY/ (confirm files arrived)
    → Cleanup: rm -rf ~/.claude/MEMORY/ (only after verification)
