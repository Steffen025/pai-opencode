...
**Full Documentation:** See `~/.opencode/PAI/TERMINALTABS.md`
...
2. Does current-work file exist? Check `~/.opencode/MEMORY/STATE/current-work.json`
...
grep '"type":"user"' ~/.opencode/projects/-Users-username--claude/*.jsonl | head -1 | jq '.type'
...
**Check:**
1. Does `~/.opencode/PAI/SKILL.md` exist?
...
- **Agent System:** `~/.opencode/skills/Agents/SKILL.md`
- **History/Memory:** `~/.opencode/PAI/MEMORYSYSTEM.md`
...
~/.opencode/hooks/                     Hook scripts (22 files)
~/.opencode/hooks/handlers/            Handler modules (6 files)
~/.opencode/hooks/lib/                 Shared libraries (13 files)
~/.opencode/hooks/lib/learning-utils.ts Learning categorization
~/.opencode/hooks/lib/time.ts          PST timestamp utilities
~/.opencode/hooks/lib/event-types.ts   Typed event definitions (22 interfaces)
~/.opencode/hooks/lib/event-emitter.ts appendEvent() → events.jsonl
~/.opencode/MEMORY/WORK/               Work tracking
~/.opencode/MEMORY/LEARNING/           Learning captures
~/.opencode/MEMORY/STATE/              Runtime state
~/.opencode/MEMORY/STATE/events.jsonl  Unified event log (append-only)

INFERENCE TOOL (for hooks needing AI):
Path: ~/.opencode/PAI/Tools/Inference.ts
...
tail -f ~/.opencode/MEMORY/STATE/events.jsonl | jq

# Filter by type
tail -f ~/.opencode/MEMORY/STATE/events.jsonl | jq 'select(.type | startswith("algorithm."))'
