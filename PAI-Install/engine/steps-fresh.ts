...
async function installAnthropicMaxBridge(
	...
	if (!existsSync(tokenBridgeSrc)) {
		return {
			success: false,
			message: `Token bridge plugin not found at ${tokenBridgeSrc}. Re-clone the repository and retry.`,
			tokenHoursRemaining: 0,
		};
	}
	...
	try {
		...
		// Validate expiresAt is a valid number in the future
		if (typeof oauth.expiresAt !== "number" || oauth.expiresAt <= Date.now()) {
			return {
				success: false,
				message: "Token expired or invalid expiry in Keychain. Run 'claude' to re-authenticate.",
				tokenHoursRemaining: 0,
			};
		}
...
export async function stepInstallPAI(
	...
	// Build env content - skip provider API key line for anthropic-max (uses OAuth, not API key)
	let envContent = `# PAI-OpenCode Environment Variables
# Generated by installer - DO NOT COMMIT THIS FILE
`;
	
	// Only add provider API key for non-anthropic-max providers
	if (state.collected.provider !== "anthropic-max") {
		envContent += `${providerEnvVar}=${state.collected.apiKey || ""}\n`;
	}
...
export async function runFreshInstall(
  ...
  if (provider === "anthropic-max") {
    // Fail fast on non-macOS platforms
    if (process.platform !== "darwin") {
      await emit({
        event: "error",
        content: "❌ Anthropic Max preset is only supported on macOS (requires Keychain access).",
      });
      throw new Error("anthropic-max preset requires macOS");
    }
    await emit({
      event: "message",
      content:
